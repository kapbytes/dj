...
def scrape():
    ...
    for country_url in country_url_list: 
        ...
        country_obj.Name = country_obj.Name.replace(' ','-')
        ...
        if(len(state_soup)>0):
            ...
            for state_l in states_links:
                ...
                state_obj.Name = state_obj.Name.replace(' ','-')
                state_obj.country_name = state_obj.country_name.replace(' ','-')
        ...
        if len(city_links)>0:
            for state_l in city_links:
                ...
                # print(country_d_city)
                city_obj.Name = city_obj.Name.replace(' ','-')
                city_obj.country_name = city_obj.country_name.replace(' ','-')
                city_obj.state_name = city_obj.state_name.replace(' ','-')
